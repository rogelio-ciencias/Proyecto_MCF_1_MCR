...
Para los incisos d), e) y f), aplicamos ventanas móviles de 252 días con el fin de evaluar dinámicamente el riesgo y realizar el backtesting de nuestros modelos. Al analizar las violaciones, observamos que mientras el modelo normal al 95% presenta una calibración aceptable con un 5.31% de excepciones, al nivel del 99% el número de fallos (2.22%) supera por más del doble la tolerancia esperada, lo que nos indica una clara subestimación del riesgo extremo y la presencia de riesgo de modelo. Incluso cuando intentamos corregir esto ajustando el VaR por volatilidad móvil en el inciso f), encontramos que las violaciones al 99% siguen siendo elevadas (2.28%), lo que nos confirma que la asimetría y las colas pesadas de nuestro activo requieren modelos más robustos que la simple distribución normal para capturar los eventos de estrés real.

Podemos concluir, en base a los resultados que hay que tener otro enfoque para analizar los rendimientos de nuestro activo. Más allá de una distribución normal necesitamos un modelo con colas pesadas o no paramétrico para tener estimaciones más precisas, para no subestimar el riesgo, pues en este caso los valores extremos toman relevancia para el modelo.
